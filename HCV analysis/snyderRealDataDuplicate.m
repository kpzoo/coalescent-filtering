...
function [xhat, Nhat, lamhat, tn, nLin, statsStruc, qn] = snyderRealDataDuplicate(fn, q0, tcoal)

% Assumptions and modifications
% - modified to treat duplicates by discontinuous updates
% - using the non-linear case only
...
xhat = qn*xsetMx';
statsStruc.xhatmean = xhat;
xhatstd = sqrt(qn*(xsetMx'.^2) - xhat.^2);
statsStruc.xhatstd = xhatstd;
statsStruc.xstdub = xhat + 2*xhatstd;
statsStruc.xstdlb = xhat - 2*xhatstd;