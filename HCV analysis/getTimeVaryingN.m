...
% - added a case 7 for log exponentual Pybus
% - added case 6 for reparameterised Pybus exponential
...
switch(id)
    ...
    case 6
        % Pybus piecewise exponential N(t) with x and y-x parameters
        I1 = t <= y{3};
        I2 = t > y{3} & t < (y{4} + y{3});
        I3 = t >= (y{4} + y{3});
        Nt = y{1}.*I1 + y{1}.*exp(-y{2}.*(t - y{3})).*I2 + ...
            y{1}.*exp(-y{2}.*y{4}).*I3;
    case 7
        % Pybus exponential but in log of population and x, y-x form
        I1 = t <= y{3};
        I2 = t > y{3} & t < (y{4} + y{3});
        I3 = t >= (y{4} + y{3});
        logNt = y{1}.*I1 + (y{1} -y{2}.*(t - y{3})).*I2 + ...
            (y{1} -y{2}.*y{4}).*I3;
        Nt = exp(logNt);
end
