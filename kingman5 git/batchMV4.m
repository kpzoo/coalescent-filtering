...
% - added skyline to see where signal is coming from
% - checked numerical effects for multimodal exponential
% - added fixed midpoint
% - works with getPopBackward to get optimal Nt estimates
% - includes N(t) from the final estimate of the parameters and bounds
% - removed special treatment of case 2 for posteriors
% - modified from batchMV2 to include snyder2 which gives qn and marginals
...
% Time logging, boolean for MSE calc, tree drawing and non-linear ODEs
tIter = zeros(1, M);
solveMSE_t = 0;
getTree = 0;
linBool = 0;
...
% Either randomly sample or take the middle value
for i = 1:numRV
    xset{i} = linspace(minSpace(i), maxSpace(i), mi(i));
    if midpt
        x(i) = xset{i}(ceil(mi(i)/2));
        fn.param = x;
    end
end
...
for ii = 1:M
    ...
    %% Pick value randomly if not midpt
    for i = 1:numRV
        if ~midpt
    ...
    % Get classical skyline plot
    if fnid == 1
        Nsky = twait.*fac;
        %Nskyts = timeseries([Nsky(1) Nsky], tcoal);
        % Repeat values to plot in zoh form
        ND = [kron(Nsky, [1 1]) Nsky(end)];
        tD = [tcoal(1) kron(tcoal(2:end), [1 1])];
        
        tcomp = linspace(tcoal(1), tcoal(end), 1000);
        Ncomp = x(1)*exp(-x(2)*tcomp);
        
        figure;
        %stairs(sort([tcoal tcoal]), [Nsky(1) Nsky]);
        semilogy(tD, ND);
        hold on
        semilogy(tcomp, Ncomp, 'k');
        hold off
        legend('skyline', 'true', 'location', 'best');
        xlabel('time');
        ylabel('population');
    end
    ...
    [xhat, Nhat, lamhat, lam, N, tn, nLin, statsStruc, qn] = snyder2(fn, q0, tcoal, linBool);
    tnmin = min(tn);
    tnmax = max(tn);
    tnlen = length(tn);
    ...
    if optimalNt
        ...
        [Nft, ~] = getTimeVaryingN(fn, tz, -1, x'); %% correction to ensure compare to original
    end
    ...
    if plotindiv
        ...
        if optimalNt
            ...
            hold off
            legend('N(t)', 'Nhat(t)', 'lower bound', 'upper bound', 'location', 'best');
            xlim([tcoal(1) tcoal(end)]);
            xlabel('time');
            ylabel('N(t) estimates and bounds');
            title('Population estimate using final parameter values');
    ...
    disp('***************************************************************');
    % Get percentage accuracy for each parameter
    perAcc = 100*(((est.xhat - est.x)./est.x).^2);
    disp(['Perc accuracy: ' num2str(perAcc)]);
...
if M > 1
    ...
    save(['noMid_' num2str(numRV) '_' num2str(M)], 'mse_t', 'param', 'paramhat',...
        'n', 'M', 'tIter', 'fn');
...
%         'Nhat', 'statsStruc', 'mse_t', 'tIter', 'C', 'tcoal', 'est');
    save(['expNew_' num2str(numRV)], 'fn', 'nLin', 'tn', 'lamhat', 'lam', 'N',...
        'Nhat', 'statsStruc', 'mse_t', 'tIter', 'C', 'tcoal', 'est');
end