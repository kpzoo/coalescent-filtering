...
% Time logging, boolean for MSE calc and tree drawing
tIter = zeros(1, M);
solveMSE_t = 0;
getTree = 0;
...
for ii = 1:M
    ...
    % Special lineage through time test plot
    if plotindiv
        figure;
        stairs(tLin, nLin);
        hold on
        plot(repmat(svec, [2 1]), [zeros(size(svec)); n*ones(size(svec))], 'r--');
        hold off
    ...
    [xhat, Nhat, lamhat, lam, N, tn, statsStruc, qn] = snyderHetero(fn, q0, events, bools);
    tnmin = min(tn);
    tnmax = max(tn);
    tnlen = length(tn);
    ...
    if plotindiv
        ...
        if optimalNt
            ...
            hold off
            legend('N(t)', 'Nhat(t)', 'lower bound', 'upper bound', 'coalescents', 'samples', 'location', 'best');
            xlim([tcoal(1) tcoal(end)]);
            xlabel('time');
            ylabel('N(t) estimatea and bounds');
...
if M > 1
    ...
    save(['hetero_' num2str(numRV)], 'fn', 'tn', 'lamhat', 'lam', 'N',...
        'Nhat', 'statsStruc', 'mse_t', 'tIter', 'C', 'est', 'events');
end