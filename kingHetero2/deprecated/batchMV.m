...
minSpace{1} = [100*n 0.1];
maxSpace{1} = [1000*n 10];
minSpace{2} = [100*n 0.1 0 1100*n];
maxSpace{2} = [1000*n 10 pi/2 1200*n];
minSpace{3} = 100*n;
maxSpace{3} = 1000*n;

% Modify spaces if specify parameters so that they are set to min values
if nEstRV > 0
    minSpace{fnid}(specParam) = maxSpace{fnid}(specParam);
end
...
% Set space and uniform prior q0
minspace = minSpace{fnid};
maxspace = maxSpace{fnid};
q0 = ones(1, m)/m;
...
for ii = 1:M
    ...
    if fnid == 2 && plotindiv
        ...
        Nhatx = xhat(end, 1)*sin(xhat(end, 2)*tx + xhat(end, 2)) + xhat(end, 4);
        % Plot results
        figure;
        plot(tx, Nx, tx, Nhatx);
        xlim([min(tx) tnmax]);
