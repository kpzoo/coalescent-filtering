%% Script that focuses on con-exp-con functions sampled across time only
...
% Get actual no. lineages at sample times
rvec = zeros(size(svec));
for i = 1:length(svec)
    rvec(i) = nLin(tLin == svec(i));
end

% Plot N(t) estimate and lineages on same plot with sample times
figure;
[hAx, hLine1, hLine2] = plotyy([tz',tz'], [Nz', Nzhat'], tLin', nLin', 'plot', 'stairs');
hold on
stem(svec, hAx(1).YLim(2)*ones(size(svec)), 'go--');
hold off
...
hLine2.Color = 'm';
grid;
limz = [0 max(tz(end), tLin(end))];
hAx(1).XLim = limz;
hAx(2).XLim = limz;
hAx(2).YLim = [min(nLin) max(nLin)];
legend('N(t)', 'Nhat(t)', 'sample times', 'lineages', 'location', 'best');
xlabel('time');
ylabel(hAx(1), 'population size') % left y-axis
ylabel(hAx(2), 'lineage number') % right y-axis
title('Comparison of demographic functions and lineages');