%% Function that compares con-exp-con and exp functions sampled across time 
function [est, est1, tz, Nz, Nz1, Nzhat, Nzhat1, extra] = pybexpDelFn(n, nSampTimes, mi)

% Assumptions and Modifications
% - uses 2 parameter form for the con-exp-con case
...
minSpace = [100000 0.01 50 50];
maxSpace = [200000 0.1 50 50];
extra.minSpace = minSpace;
extra.maxSpace = maxSpace;
...
% Extra x values to offset exponential to con-exp-con
if numRV == 2
    x = [x maxSpace(3:4)];
end
...
if numRV == 4
    ...
    [xhat, xhatmean, xstdlb, xstdub, qnlast] = pybexpFilterDel(mi, nData, 0, xsetMx, tLin, nLin, facSort, svec, delRV);
end
...
fn.id = 6;
if delRV.bool
    [Nzhat, Nbnd] = getPopBackwardDel(tz, qnlast, fn, delRV);
else
